...
const createToken = user => {  // get user and return token
  return jwt.sign({ username: user.username, _id: user._id }, jwtConfig.secret, { expiresIn: 60 * 60 * 60 });
};

const createUser = async (user, response) => {  // get user credentials, store user and respond with token
  try {
...
router.post('/login', async (ctx) => {  // get user's credentials and, if they are correct, respond with token
